...
class Robot:
    ...
    def initialize_state_array(self):
        ...
        self.state = np.full((self.num_fov_pixels, 2, 1 + 3),
                             np.nan,np.single)  # 2, because we take two last frames. 1 is the distance to nearest object. 3 is number of object types (wall,robot,door)  Robo
    ...
    def get_observed_state(self, room, without_update_self=False):
        ...
        self.update_state_with_wall_or_door(state,wall_bottom_left,wall_bottom_right,self.location[1],270-angle_bin_centers,angle_bin_edges)
        self.update_state_with_wall_or_door(state,wall_bottom_right, wall_top_right, room.width - self.location[0],angle_bin_centers, angle_bin_edges)
        self.update_state_with_wall_or_door(state,wall_top_left, wall_bottom_left, self.location[0],180-angle_bin_centers, angle_bin_edges)
        self.update_state_with_wall_or_door(state,wall_top_right, wall_top_left,room.height - self.location[1],90-angle_bin_centers, angle_bin_edges)
        self.update_state_with_wall_or_door(state,door_right,door_left,room.height - self.location[1], 90-angle_bin_centers, angle_bin_edges,is_door=True)
