...
class Animator:
    ...
    def __init__(self,room:Room, robots_to_debug=None):
        ...
        self.circles = [plt.Circle((0, 0), robot.radius, color='green' if robot in self.robots_to_debug  else 'blue') for robot in room.robots]
        self.dirs = [plt.plot([0, 0], [0, 0], color='yellow',linewidth=2)[0] for _ in room.robots]
        self.views = [[plt.plot([0, 0], [0, 0], color='silver', linestyle='-',linewidth=0.4)[0] for _ in range(robot.num_fov_pixels)] for robot in room.robots if robot in self.robots_to_debug]
    ...
    def Update(self):
        ...
        for j,robot in enumerate(self.room.robots):
            ...
            if robot in self.robots_to_debug:
                ...
                angle_bin_edges = np.deg2rad(np.linspace(start=limit_right, stop=limit_left,num=robot.num_fov_pixels + 1))
